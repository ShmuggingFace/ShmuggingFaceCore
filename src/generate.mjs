...
function normalizeConfig(config, options = {}) {
  ...
  const model = {
    ...
    datasets: datasets.map((dataset, index) => {
      ...
      const allFiles = [...normalizedFiles, ...artifactGroups.flatMap((group) => group.files)];
...
function kaggleDataExplorer(dataset, rowCount) {
  ...
  return `<section class="kg-data-explorer" data-kg-explorer>
    <div class="kg-de-main">
      ...
      <div class="kg-de-toolbar">
        ...
        <button type="button" class="kg-de-column-count" data-kg-column-menu><span data-kg-column-count>${escapeHtml(String(visibleCount))}</span> of ${escapeHtml(String(columns.length))} columns⌄</button>
        <div class="kg-de-column-menu" data-kg-column-popover hidden>
          ${columns.map((column, index) => `<label><input type="checkbox" data-kg-column-toggle="${index}" ${index < visibleCount ? "checked" : ""}> ${escapeHtml(column)}</label>`).join("")}
        </div>
      </div>
  ...
  <script>
    (() => {
      ...
      const columnCount = explorer.querySelector("[data-kg-column-count]");
      const syncColumns = () => {
        const toggles = [...explorer.querySelectorAll("[data-kg-column-toggle]")];
        const checked = toggles.filter((input) => input.checked);
        if (!checked.length && toggles.length) {
          toggles[0].checked = true;
          checked.push(toggles[0]);
        }
        const visible = new Set(checked.map((input) => input.dataset.kgColumnToggle));
        explorer.querySelectorAll("[data-kg-col]").forEach((cell) => {
          cell.toggleAttribute("data-kg-hidden", !visible.has(cell.dataset.kgCol));
        });
        if (columnCount) columnCount.textContent = String(visible.size);
      };
      explorer.querySelectorAll("[data-kg-column-toggle]").forEach((input) => input.addEventListener("change", syncColumns));
...
function files(dataset) {
  ...
  if (!dataset.artifactGroups?.length) return dataset.files.map(renderFile).join("");
  return dataset.artifactGroups
...
function datasetFiles(dataset) {
  return dataset.allFiles?.length ? dataset.allFiles : dataset.files;
}

function rowCountForSplit(dataset, split, fallbackRowCount) {
  return dataset.splitRowCounts?.[split] ?? fallbackRowCount;
}
...
function viewerShell(dataset, { studio = false } = {}) {
  ...
  const activeSplitRows = rowCountForSplit(dataset, activeSplit, rowCount);
  return `<section class="dataset-viewer ${studio ? "studio-dataset-viewer" : ""}" id="data-studio" data-viewer="${escapeHtml(dataset.slug)}">
    ...
    <div class="viewer-menu-row">
      ...
      <div class="viewer-menu" data-menu="split" hidden>${splits.map((split) => `<button type="button">${escapeHtml(split)} · ${formatNumber(rowCountForSplit(dataset, split, rowCount))} rows</button>`).join("")}</div>
    </div>
...
function renderFileBrowser(dataset) {
  ...
  const groupedFileRows = dataset.artifactGroups?.length
    ? dataset.artifactGroups.map((group) => `<div class="repo-file-group"><div class="repo-file-group-heading"><strong>${escapeHtml(group.title)}</strong><span>${escapeHtml(group.description || `${group.files.length} files`)}</span></div>${group.files.map(renderFileRow).join("")}</div>`).join("")
...
function renderHfDataStudio(model, dataset) {
  ...
  const splitPills = dataset.subsets.flatMap((subset) => dataset.splits.map((split, index) => [
    `${subset}/${split}`,
    formatNumber(rowCountForSplit(dataset, split, index === 0 ? rowCount : Math.max(1, Math.round(rowCount * 0.1)))),
  ]));
...
function renderKaggle(model, dataset, activeTab = "data-card") {
  const rowCount = Math.max(dataset.rowCount, dataset.rows.length, 1);
  const filesForDataset = datasetFiles(dataset);
  const firstFile = filesForDataset.find((file) => !file.downloadUrl) || filesForDataset[0];
  ...
  const sidebarItems = model.datasets
    ...
    .join("");
  const fileRows = (dataset.artifactGroups?.length ? dataset.artifactGroups : [{ title: "Files", files: dataset.files }])
    .map((file) => {
      if (file.files) {
        return `<li class="kg-file-group"><strong>${escapeHtml(file.title)}</strong><ul>${file.files.map((groupFile) => {
  ...
  const cover = coverPath
    ...
    : "";
  const firstTabularPath = filesForDataset.find((file) => /\.(csv|tsv)$/i.test(file.path) && !file.downloadUrl)?.path
  ...
  const body = `<div class="kg-shell" data-kaggle-page="${escapeHtml(dataset.slug)}">
    ...
    <main class="kg-main">
      ...
      <header class="kg-sticky-repo-header" aria-label="Sticky dataset header">
        <div class="kg-sticky-repo-row">
          <a class="kg-sticky-title" href="/kaggle/${dataset.slug}/">🚀<strong>${escapeHtml(dataset.title)}</strong></a>
          <div class="kg-sticky-actions">
        ...
        <nav class="kg-sticky-tabs" aria-label="Sticky dataset tabs">${stickyTabs}</nav>
      </header>
  ...
  <script>
    ...
    page.querySelectorAll("[data-kg-code]").forEach((button) => toggle(button, page.querySelector("[data-kg-code-popover]")));
    page.querySelectorAll("[data-kg-more]").forEach((button) => toggle(button, page.querySelector("[data-kg-more-popover]")));
    ...
    page.querySelectorAll(".kg-score").forEach((score) => score.addEventListener("click", (event) => {
      event.currentTarget.classList.toggle("active");
      showToast(event.currentTarget.classList.contains("active") ? "Mock upvote recorded." : "Mock upvote removed.");
    }));
    const syncStickyHeader = () => document.body.classList.toggle("kg-scrolled", window.scrollY > 210);
    syncStickyHeader();
    ...
    const setMetadataOpen = (button, open) => {
      const panel = document.getElementById(button.getAttribute("aria-controls"));
      button.setAttribute("aria-expanded", String(open));
      button.classList.toggle("open", open);
      panel?.toggleAttribute("hidden", !open);
    };
    page.querySelectorAll("[data-kg-metadata-toggle]").forEach((button) => button.addEventListener("click", () => {
      setMetadataOpen(button, button.getAttribute("aria-expanded") !== "true");
    }));
    page.querySelector("[data-kg-metadata-collapse]")?.addEventListener("click", () => {
      page.querySelectorAll("[data-kg-metadata-toggle]").forEach((button) => setMetadataOpen(button, false));
    });
    page.querySelector("[data-kg-about-suggest]")?.addEventListener("click", () => showToast("Mock suggestion action is locked for review."));
...
function renderKaggleTabContent(dataset, activeTab, { rowCount, fileRows, tags, codeSnippet, cover }) {
  ...
  const splitSummary = dataset.splits
    .map((split) => `${split}: ${formatNumber(rowCountForSplit(dataset, split, rowCount))}`)
    .join(", ");
  const metadataAccordions = [
    ...
    ["Coverage", `<p>${escapeHtml(formatNumber(rowCount))} invented rows across ${escapeHtml(splitSummary || "configured splits")} with ${escapeHtml(String(dataset.columns.length))} schema columns.</p>`],
  ...
  return `<section class="kg-content-grid">
    ...
    <section class="kg-social-proof">
      ...
      <div class="kg-survey-stack">
        <section>
          <h3>What have you used this dataset for?</h3>
          <div class="kg-survey-pills"><button type="button">Learning 9</button><button type="button">Research 6</button><button type="button">Application 3</button><button type="button">LLM Fine-Tuning 0</button></div>
        </section>
        <section>
          <h3>How would you describe this dataset?</h3>
          <div class="kg-survey-pills"><button type="button">Well-documented 1</button><button type="button">Well-maintained 1</button><button type="button">Clean data 1</button><button type="button">Original 1</button><button type="button">High-quality notebooks 1</button><button type="button">Other</button></div>
        </section>
      </div>
    </section>
    <section class="kg-metadata-block">
      <div class="kg-metadata-heading">
        <div><span>▤</span><h2>Metadata</h2></div>
        <button type="button" data-kg-metadata-collapse><span>⌄⌃</span> Collapse All</button>
      </div>
      ${metadataAccordions.map(([title, content], index) => `<div class="kg-metadata-item">
        <button type="button" class="kg-metadata-toggle ${index === 0 ? "open" : ""}" data-kg-metadata-toggle aria-expanded="${index === 0 ? "true" : "false"}" aria-controls="kg-metadata-panel-${index}">
          <strong>${escapeHtml(title)}</strong><span>⌄</span>
        </button>
        <div class="kg-metadata-panel" id="kg-metadata-panel-${index}"${index === 0 ? "" : " hidden"}>
          ${content}
        </div>
      </div>`).join("")}
    </section>
...
function kagglePrecisionStyles() {
  return `
