...
function renderKaggleTabContent(dataset, activeTab, { rowCount, fileRows, tags, codeSnippet }) {
  ...
  const metadataAccordions = [
    ["Collaborators", `Dataset owner: ${dataset.owner}. Review access is mocked for this Shmaggle preview only.`],
    ["Authors", `${dataset.contactName || "Maya Lintwell"} maintains this mock release. Contact: ${dataset.contactEmail || "reviewer@example.test"}.`],
    ["Coverage", `${formatNumber(rowCount)} invented rows across train, validation, and test splits with ${dataset.columns.length} schema columns.`],
    ["DOI Citation", `No real DOI. Cite this as a prerelease mock for ${dataset.owner}/${dataset.slug}.`],
    ["Provenance", "Rows are synthetic fixtures generated for UI review. They do not describe a real dataset or real people."],
    ["License", dataset.license],
  ];
  if (activeTab === "code") {
  ...
  return `<section class="kg-content-grid">
    ...
    <section class="kg-metadata-block">
      ...
      ${metadataAccordions.map(([title, content], index) => `<div class="kg-metadata-item">
        <button type="button" class="kg-metadata-toggle" data-kg-metadata-toggle aria-expanded="false" aria-controls="kg-metadata-panel-${index}"><span>⌄</span>${escapeHtml(title)}</button>
        <div class="kg-metadata-panel" id="kg-metadata-panel-${index}" hidden>
